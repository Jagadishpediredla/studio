...
export default function AppHeader({ pipelineStatus, onCompile, className, ...props }: AppHeaderProps) {
  ...
  return (
    <header className={cn("flex items-center justify-between p-4 rounded-lg border bg-card text-card-foreground shadow-sm", className)} {...props}>
      ...
      <div className="flex items-center gap-2">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => onCompile('compile')} disabled={isActionInProgress}>
              <Cog className="mr-2 h-4 w-4" /> Compile Firmware
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onCompile('upload')} disabled={isActionInProgress}>
