...
export default function NavRail({ onNavAction }: NavRailProps) {
    ...
    return (
        <>
            ...
            <aside className="fixed left-0 top-0 h-full bg-card border-r flex flex-col items-center justify-between p-2 z-50 hidden sm:flex">
                <div>
                     <TooltipProvider>
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-card border-t flex items-center justify-around p-2 z-50 sm:hidden">
                {navItems.map((item) => (
                    <Tooltip key={item.label}>
                        <TooltipTrigger asChild>
                            {item.href ? (
                                <Link href={item.href} className="flex flex-col items-center justify-center p-2 text-muted-foreground transition-colors hover:text-foreground">
                                    <item.icon className="h-5 w-5" />
                                    <span className="text-xs mt-1">{item.label}</span>
                                </Link>
                            ) : (
                                <button onClick={() => onNavAction(item.action as any)} className="flex flex-col items-center justify-center p-2 text-muted-foreground transition-colors hover:text-foreground">
                                    <item.icon className="h-5 w-5" />
                                    <span className="text-xs mt-1">{item.label}</span>
                                </button>
                            )}
